...
from src.api.models import Subscriber, WikiLink
from src.utils.get_emails import link_email, subscribed_email
...
def send_email_to_subscribers() -> None:
    ...
    if subscribers:
        wiki_link = (
            db_session.query(WikiLink).filter(WikiLink.date_used == None).first()
        )
